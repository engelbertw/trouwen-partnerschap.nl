...
export default function Partner1NaamgebruikPage(): JSX.Element {
    ...
    const fetchPartners = async () => {
      try {
        ...
        if (result.success) {
          ...
          setSelectedKeuze(result.partner1?.naamgebruikKeuze || null);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        router.push(`/dossier/${dossierId}/naamgebruik/partner2`);
      } else {
  ...
  const getDisplayName = (keuze: NaamgebruikKeuze): string => {
    ...
    const eigen = partner1.voorvoegsel 
      ? `${partner1.voorvoegsel} ${partner1.geslachtsnaam}` 
      : partner1.geslachtsnaam;
    const partnerNaam = partner2.voorvoegsel 
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
          <h2 className="font-serif text-2xl font-bold text-gray-900 mb-2">
            Naamgebruik {getFullName(partner1)}
          </h2>
